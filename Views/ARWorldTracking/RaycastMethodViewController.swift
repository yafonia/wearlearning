//
//  RaycastMethodViewController.swift
//  WeAReLearning
//
...
// A centered, non-draggable dialog shown before the Add Anchor dialog. Lets the user
// pick how the tap is turned into a world position. onContinue delivers the chosen
// RaycastMethod.
final class RaycastMethodViewController: UIViewController {
    private let onContinue: (RaycastMethod) -> Void
    private var selectedMethod: RaycastMethod = .arViewRaycast
    ...
    init(onContinue: @escaping (RaycastMethod) -> Void) {
    ...
    private func setupContent() {
        ...
        titleLabel.text = "Raycast method"
        titleLabel.font = .preferredFont(forTextStyle: .headline)

        let optionsStack = UIStackView()
        optionsStack.axis = .vertical
        optionsStack.spacing = 4
        for method in RaycastMethod.allCases {
            let button = makeOptionButton(for: method)
    ...
    private func makeOptionButton(for method: RaycastMethod) -> UIButton {
        var config = UIButton.Configuration.plain()
        config.title = method.title
    ...
    private func updateOptionSelection() {
        for (index, method) in RaycastMethod.allCases.enumerated() {
            let isSelected = method == selectedMethod
            optionButtons[index].configuration?.image =
                UIImage(systemName: isSelected ? "largecircle.fill.circle" : "circle")
        }
    }
